Public Class MAINTENANCE_BOSKP
    ...
    Private Sub MAINTENANCE_BOSKP_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Me.Text = "BOSKP-Showroom " & gs_version2
        Me.CenterToScreen()
        btnUpdate.Hide()
    End Sub
    ...
    Private Sub btnViewAll_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnViewAll.Click
        ...
        Dim SQLTXT As String = "select * from boskp where class_01 not like 'Showroom/NCR' order by boscode asc"
    ...
    Private Sub btnEdit_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnEdit.Click
        LS_BCODE = InputBox("Please input branch code.", "BOSKP - Vismin Maintenance")
        If LS_BCODE = "" Then
        Else
            findbranch()
        End If
        ...
        Else
            If Me.alreadysave = False Then
                ...
                If rply = vbYes Then
                    ...
                    " VALUES ('" + Me.txtBosCode.Text + "','" + Me.txtKpCode.Text + "','" + Me.txtBranchName.Text + "','VISMIN','VISMIN','VISMIN','VISMIN')"
                    Dim rdr As SqlClient.SqlDataReader = Nothing
                    If c.Error_Inititalize_INI Then Exit Sub
                    If c.ErrorConnectionReading(False) Then Exit Sub
                    If c.Error_SetRdr(SQLTXT, rdr, sqlmsg) Then Exit Sub
                    MsgBox("Branch Save in Synergy VisMin", 64, "BOSKP")
    ...
    Public Function alreadysave() As Boolean
        Dim c As New clsData
        Dim SQLTXT As String = "SELECT * FROM BOSKP WHERE BOSCODE = '" + txtBosCode.Text + "' AND class_01 not like 'Showroom/NCR'"
        Dim rdr As SqlClient.SqlDataReader = Nothing
    ...
    Public Sub findbranch()
        Dim c As New clsData
        Dim SQLTXT As String = "SELECT * FROM BOSKP WHERE BOSCODE = '" + LS_BCODE + "' AND class_01 not like 'Showroom/NCR'"
        Dim rdr As SqlClient.SqlDataReader = Nothing
        If c.Error_Inititalize_INI Then Exit Sub
        If c.ErrorConnectionReading(False) Then Exit Sub
        If c.Error_SetRdr(SQLTXT, rdr, sqlmsg) Then Exit Sub
...
"where boscode = '" + Me.txtBosCode.Text + "'"
            ...
            If c.ErrorConnectionReading(False) Then Exit Sub
            If c.Error_SetRdr(SQLTXT, rdr, sqlmsg) Then Exit Sub
            MsgBox("Branch updated in Synergy VisMin", 64, "BOSKP")
            Exit Sub
        Else
            Exit Sub
        End If
