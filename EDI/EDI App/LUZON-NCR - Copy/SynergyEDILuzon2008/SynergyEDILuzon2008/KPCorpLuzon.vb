Public Class KPCorpLuzon
    ...
    Private Sub KPCorpLuzon_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
        Dim main As New EDI_Luzon_Main
        main.Show()
    End Sub

    Private Sub KPCorpLuzon_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Me.Text = "KP Corp-" & gs_serverloc & " " & gs_version
...
continuesave:
            ...
            sqltxt = "select * from kp_corp where bcode = 000 and date_time > '" & _
            ...
            For n = 0 To grd.Rows - 2
                If Not LCase(grd.get_TextMatrix(n, 0).ToString) = "totals" Then
                    ...
                    sqltxt = "select * from boskp where kpcode = '" + grd.get_TextMatrix(n, 1) + "'" 'original code
                    ...
                    If rdr.Read Then
                        ...
                        For r = 1 To 2
                            ...
                            If r = 1 Then
                                amount = FormatNumber(grd.get_TextMatrix(n, 4), 2)
                            ...
                            Else
                                ...
                                acfc = FormatNumber(totalAmount, 2)
                                'MsgBox(acfc)
                                CompanyAccountCode = "4200003"
...
extcase:
                        Next r
                        sqltxt = "Insert into kp_corp(bcode, month_eli, year_eli, " & _
            ...
            Dim act As String = "EDI" & " " & "KP Corp" & gs_serverloc & "for" & " " & txtdesc.Text.Trim
...
extAlreadySave:
    ...
    Private Function alreadysave() As Boolean
        ...
        Dim n As Integer = Nothing
        Dim squery1 As String = Nothing
        Dim squery2 As String = Nothing
        For n = 0 To grd.Rows - 2
            Dim g As String = grd.get_TextMatrix(n, 1)
            squery1 = "select * from boskp where kpcode = '" + grd.get_TextMatrix(n, 1) + "'" 'original code
            If c.Error_Inititalize_INI Then Exit Function
            If c.ErrorConnectionReading(False) Then Exit Function
            If Not c.Error_SetRdr(squery1, rdr, sqlmsg) Then
                While rdr.Read
                    bcode = rdr(0)
                    squery2 = "Select * from kp_corp where month_eli = '" & cboperiod.Text & "' and year_eli = '" & cboyear.Text & "' and bcode = '" & bcode & "'"
                End While
                rdr.Close()

                If Not c.Error_SetRdr(squery2, rdr, sqlmsg) Then
                    While rdr.Read
                        alreadysave = True
                        Exit Function
                    End While
...
EXT:
    ...
    Private Sub txtdesc_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtdesc.TextChanged
        Label8.Text = txtdesc.Text & " " & "KP Corp"
    End Sub
End Class