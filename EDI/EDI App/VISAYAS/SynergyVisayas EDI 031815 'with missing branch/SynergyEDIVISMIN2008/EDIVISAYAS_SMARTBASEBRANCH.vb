...
Public Class EDIVISAYAS_SMARTBASEBRANCH
...
extloop:
        ...
        response = JsonConvert.DeserializeObject(service.insertTransactionWithDesc(JsonConvert.SerializeObject(transactiondata), JsonConvert.SerializeObject(editabledata), "smart3_corp", "companydesc", edi), GetType(Response))
        If response.responseCode = ResponseCode.Error Then
            MessageBox.Show(response.responseMessage, "Error Saving")
            btnGIF.Hide()
            grp.Hide()
            Exit Sub
        End If
        ...
        Dim act As String = "EDI" & " " & "SMARTBASEBRANCH" & " " & "for" & " " & txtDesc.Text
        Call CheckBC_Errors()
        Try
...
extalreadysave:
    ...
    Private Function alreadysave() As Boolean
        ...
        For n = 0 To grd.Rows - 2
            ...
            Try
                response = JsonConvert.DeserializeObject(service.alreadySavedBase(g, cboPeriod.Text, cboYear.Text, "smart3_corp", edi), GetType(Response))
            Catch ex As Exception
                MessageBox.Show(ex.Message)
                Me.Close()
            End Try
