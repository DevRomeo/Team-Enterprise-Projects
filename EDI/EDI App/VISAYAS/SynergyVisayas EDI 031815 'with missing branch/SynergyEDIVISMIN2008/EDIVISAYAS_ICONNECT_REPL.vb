...
extloop:
        ...
        Next
        Try
            response = JsonConvert.DeserializeObject(service.insertTransactionBase(JsonConvert.SerializeObject(transactiondata), JsonConvert.SerializeObject(editabledata), "ICONNECT_REPLENISH", edi), GetType(Response))
        Catch ex As Exception
        ...
        Dim act As String = "EDI" & " " & "Iconnect Replenish" & " " & "for" & " " & txtDesc.Text
        Call CheckBC_Errors()
        Try
...
extalreadysave:
    ...
    Private Function alreadysave() As Boolean
        ...
        For n = 0 To grd.Rows - 2
            ...
            Try
                response = JsonConvert.DeserializeObject(service.alreadySavedBase(g, cboPeriod.Text, cboYear.Text, "ICONNECT_REPLENISH", edi), GetType(Response))
            Catch ex As Exception
                MessageBox.Show(ex.Message)
                Me.Close()
            End Try
