...
extcase:
            ...
            Else
                Try
                    response = toResponse(service.insertToWu_unknownbc(bcode, Format(CDbl(grd.get_TextMatrix(n, 3)), "##0.00"), Format(CDbl(grd.get_TextMatrix(n, 4)), "##0.00"), edi))
                Catch ex As Exception
                    MessageBox.Show(ex.Message)
                    btnGIF.Hide()
                    grp.Hide()
...
extloop:
        ...
        Next
        Try
            response = JsonConvert.DeserializeObject(service.insertTransactionBase(JsonConvert.SerializeObject(transactiondata), JsonConvert.SerializeObject(editabledata), "MoneyGram_corp", edi), GetType(Response))
        Catch ex As Exception
        ...
        Dim act As String = "EDI" & " " & "MoneyGram" & " " & "for" & " " & txtDesc.Text
        Call CheckBC_Errors()
...
extalreadysave:
    ...
    Private Function alreadysave() As Boolean
        ...
        For n = 0 To grd.Rows - 2
            ...
            Try
                response = JsonConvert.DeserializeObject(service.alreadySavedBase(g, cboPeriod.Text, cboYear.Text, "MoneyGram_corp", edi), GetType(Response))
            Catch ex As Exception
                MessageBox.Show(ex.Message)
                Me.Close()
            End Try
