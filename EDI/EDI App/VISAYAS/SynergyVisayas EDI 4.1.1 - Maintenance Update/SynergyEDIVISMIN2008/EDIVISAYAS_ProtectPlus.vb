...
isActionedRetry:
                ...
                For r = 1 To 6
                    ...
                    Select Case r
                        Case 1
                            amount = Format(grd.get_TextMatrix(n, 4))
                            If amount <> 0 Then
                                acfc = 0
                        ...
                        Case 2
                            ...
                            End If
                        Case 3
                            acfc = Format(grd.get_TextMatrix(n, 6))
                            If acfc <> 0 Then
                                amount = 0
                            ...
                            End If
                        Case 4
                            amount = Format(grd.get_TextMatrix(n, 8))
                            If amount <> 0 Then
                                acfc = 0
                                CompanyAccountCode = "5530001"
                            Else
                            ...
                            End If
                        Case 5
                            amount = Format(grd.get_TextMatrix(n, 9))
                            If amount <> 0 Then
                                acfc = 0
                                CompanyAccountCode = "3100007"
                            Else
                            ...
                            End If
                        Case 6
                            acfc = Format(grd.get_TextMatrix(n, 10))
                            If acfc <> 0 Then
                                amount = 0
...
extloop:
        ...
        Try
saveRetry:
            response = JsonConvert.DeserializeObject(service.insertTransactionBase(JsonConvert.SerializeObject(transactiondata), JsonConvert.SerializeObject(editabledata), "protectplus_corp", edi), GetType(Response))
        Catch ex As Exception
            If (retryCount < maxretry) Then
                retryCount += 1
                GoTo saveRetry
...
extalreadysave:
    ...
    Private Function alreadysave() As Boolean
        ...
        For n = 0 To grd.Rows - 2
            ...
            Try
alreadysavedRetry:
                response = JsonConvert.DeserializeObject(service.alreadySavedBase(g, cboPeriod.Text, cboYear.Text, "protectplus_corp", edi), GetType(Response))
            Catch ex As Exception
                If (retryCount < maxretry) Then
                    retryCount += 1
...
getMissingBranch:
    ...
    Private Function dualbranch() As Boolean
        ...
        For n = 0 To grd.Rows - 1
            Dim loc1 As String = grd.get_TextMatrix(n, 0)
...
ext:
    ...
    Private Sub Label8_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Label8.Click
        MsgBox("Debit - 3130127, Credit - 4510001", 64, "Synergy EDI-" + ediname)
        Exit Sub
    End Sub
    ...
    Private Sub txtDesc_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtDesc.TextChanged
        Label7.Text = txtDesc.Text & " " & "Protect Plus"

    End Sub

    Private Sub EDIVISAYAS_ProtectPlus_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
        Main.Show()
    End Sub
