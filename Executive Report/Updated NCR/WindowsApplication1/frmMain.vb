...
#End Region
    ...
    Private Sub GetDailyLuzon()
        ...
        Dim sql As String = "BEGIN Select DISTINCT EmailAddress,Classification,Specific,ReportID,CC,valueParamClass_01,valueParamClass_02,valueParamClass_03,valueParamClass_04,valueParamBranchCode from tbl_Recipients_V3 where Schedule='Daily' AND ValueParamClass_01='NCR' ORDER BY reportID END"
    ...
    Private Sub LoadSettings()
        Try
            ...
            'LUZON SERVER
            li_ret = GetPrivateProfileString("NCR SERVER", "SERVER", "", ls_buff, 256, ps_ini)
            ps_DBServer = ls_buff.ToString
            ps_DBServer = ps_DBServer.Substring(0, li_ret)

            li_ret = GetPrivateProfileString("NCR SERVER", "DBNAME", "", ls_buff, 256, ps_ini)
            ps_DBName = ls_buff.ToString
            ps_DBName = ps_DBName.Substring(0, li_ret)

            li_ret = GetPrivateProfileString("NCR SERVER", "USERNAME", "", ls_buff, 256, ps_ini)
            ps_DBUsername = ls_buff.ToString
            ps_DBUsername = ps_DBUsername.Substring(0, li_ret)

            li_ret = GetPrivateProfileString("NCR SERVER", "PASSWORD", "", ls_buff, 256, ps_ini)
