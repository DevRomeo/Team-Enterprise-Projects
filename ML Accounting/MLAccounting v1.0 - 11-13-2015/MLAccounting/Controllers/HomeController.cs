...
namespace AccountingWeb.Controllers
{
    ...
    {        
        ...
        {            
            ...
            var resp = srvc.UnprocessEntries(dt);
        ...
        public ActionResult ProcessEntry(UnproccessEntryModel unprocess)
        {
            if (ModelState.IsValid && unprocess.EntryDetails.Count(m => m.Process == true) > 0)
            {
                ...
                var rsponse = srvc.ProcessEntry(forProcessing);
                if (rsponse.respcode == 1)
                {

                    return Json(new { code = rsponse.respcode, msg = rsponse.message, action = Url.Action("Index", "Home") }, JsonRequestBehavior.AllowGet);
                }
                else
                {
                    return Json(new { msg = rsponse.message }, JsonRequestBehavior.AllowGet);
                }


            }
            return Json(new { msg = "Please select Data Entry to be process" }, JsonRequestBehavior.AllowGet);
        }
